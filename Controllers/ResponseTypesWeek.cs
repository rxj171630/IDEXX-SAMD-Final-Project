...
namespace AspNetCoreVueStarter
{
    ...
    [ApiController]
    public class ResponseTypesWeek
    {
            ...
            */
            ...
            if (name.CompareTo("\"All\"") == 0)
            {
                reply = GetOtherTelemetry("", "", "", "query", "query=requests" +
                    "| where timestamp > ago(7d) " + "and customDimensions contains \"GroupName\" | summarize count() | extend a = \"b\" | join" +
                    "(requests  | where timestamp > ago(7d) " + "and customDimensions contains \"GroupName\" | summarize count() by resultCode | extend  a = \"b\") on a" +
            ...
            // If the option selected is anything that is not "Other Partners", make a query for the specified partner group
            else
            {
                reply = GetOtherTelemetry("", "", "", "query", "query=requests" +
