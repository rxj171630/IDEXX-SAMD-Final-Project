...
namespace AspNetCoreVueStarter
{
    ...
    public class RequestGraph3
    {
        ...
        {
            string reply = "";
            //string reply = GetOtherTelemetry("", "", "", "query", "query=requests | where timestamp > ago(30d) | summarize avg(duration) by bin(timestamp, 1d)");
...
*/
            ...
            if (name.CompareTo("\"All\"") == 0)
            {
                reply = GetOtherTelemetry("", "", "", "query", "query=requests" +
                    "| where timestamp > ago(30d) " + "and customDimensions contains \"GroupName\" | summarize avg(duration) | extend a = \"b\" | join" +
                    "(requests  | where timestamp > ago(30d) " + "and customDimensions contains \"GroupName\" | summarize avg(duration) by bin(timestamp,1d) | extend  a = \"b\") on a" +
            ...
            {
                reply = GetOtherTelemetry("", "", "", "query", "query=requests" +
                "| where timestamp > ago(30d) " + "and customDimensions contains \"GroupName\"" +
                ...
                "(requests  | where timestamp > ago(30d) " + "and customDimensions contains \"GroupName\"" +
                " and customDimensions.GroupName != \"All\" and customDimensions.GroupName != \"Regression Cornerstone Group\"" +
                " and customDimensions.GroupName != \"Communicator Test\" and customDimensions.GroupName != \"QA CIS BATCH TEST\"" +
                "| summarize avg(duration) by bin(timestamp,1d) | extend  a = \"b\") on a" +
                "| project timestamp, avg_duration1");
            }
            ...
            {
                reply = GetOtherTelemetry("", "", "", "query", "query=requests" +
                    "| where timestamp > ago(30d) " + "and customDimensions contains \"GroupName\" and customDimensions.GroupName == " + name + " " +
                    "| summarize avg(duration) | extend a = \"b\" | join" +
                    "(requests  | where timestamp > ago(30d) " + "and customDimensions contains \"GroupName\" and customDimensions.GroupName == " + name + " " +
                    "| summarize avg(duration) by bin(timestamp,1d) | extend  a = \"b\") on a" +
                    "| project timestamp, avg_duration1");
            }
