...
namespace AspNetCoreVueStarter
{
    ...
    [ApiController]
    public class RequestsByEndpoint2
    {
        ...
        {
            string reply = "";
            //string reply = GetOtherTelemetry("", "", "", "query", "query=requests | where name contains \"Patient\" | where customDimensions.Controller contains \"\" | where timestamp > ago(7d) | summarize endpointcount = count() by bin(timestamp, 30d), tostring(customDimensions.Controller)");
            ...
            */
            ...
            if (name.CompareTo("\"All\"") == 0)
            {
                reply = GetOtherTelemetry("", "", "", "query", "query=requests" +
                    "| where timestamp > ago(7d) " + "and customDimensions contains \"GroupName\" " + "and customDimensions contains \"Controller\"" +
                    "| summarize endpoint = count() | extend a = \"b\" | join" +
                    "(requests  | where timestamp > ago(7d) " + "and customDimensions contains \"GroupName\" " + "and customDimensions contains \"Controller\"" +
                    "| summarize endpoint = count() by bin(timestamp,14d), tostring(customDimensions.Controller) | extend  a = \"b\") on a" +
            ...
            {
                reply = GetOtherTelemetry("", "", "", "query", "query=requests" +
                    "| where timestamp > ago(7d) " + "and customDimensions contains \"GroupName\" and customDimensions.GroupName == " + name + " " +
                    "and customDimensions contains \"Controller\"" +
                    "| summarize endpoint = count() | extend a = \"b\" | join" +
                    "(requests  | where timestamp > ago(7d) " + "and customDimensions contains \"GroupName\" and customDimensions.GroupName == " + name + " " +
                    "and customDimensions contains \"Controller\"" +
                    "| summarize endpoint = count() by bin(timestamp,14d), tostring(customDimensions.Controller) | extend  a = \"b\") on a" +
                    "| project timestamp, customDimensions_Controller, endpoint1" +
                    "| top 5 by endpoint1 desc");
