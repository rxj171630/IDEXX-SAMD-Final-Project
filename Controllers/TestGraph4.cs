...
namespace AspNetCoreVueStarter
{
    ...
    public class TestGraph4
    {
        ...
        public String Get()
        {
            ...
            string reply = GetOtherTelemetry("", "", "", "query", "query=exceptions | where timestamp > ago(30d) | summarize count() by bin(timestamp, 1d)");
            ...
            foreach (List<object> item in parsed.tables.ElementAt(0).rows)
            {
                ...
                temp.except_count = (long)item.ElementAt(1);
